// Main window for the PhotoCastle app.
...
void MainWindow::addZoomCombo()
{
    ...
    QStringList list(QStringList() << "" << "10%" << "25%" << "33%" << "50%" << "66%" << "75%" << "100%" << "200%" << "300%" << "400%" << "500%" << "600%" << "700%");
...
void MainWindow::batchProcess_batchProgress(int index,int total)
{
    if(index < total)
    {
        this->setWindowTitle(QString("PhotoCastle - %1/%2 (%3%)").arg(index).arg(total).arg(int(100 * static_cast<float>(index)/static_cast<float>(total))));
    }
    else
    {
        this->setWindowTitle(QString("PhotoCastle"));
    }
}
